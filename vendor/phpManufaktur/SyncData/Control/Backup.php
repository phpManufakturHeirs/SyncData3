        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                foreach ($row as $key => $value) {
                    // check encoding
                    // important to avoid null values in json_decode
                    if(!mb_detect_encoding($value,'UTF-8',true))
                    {
$this->app['monolog']->addInfo("encoding for -$value-: ".mb_detect_encoding($value,'UTF-8, ISO-8859-1, WINDOWS-1251'));
$this->app['monolog']->addInfo(sprintf("key [%s], before encoding [%s]", $key, $value),
array('method' => __METHOD__, 'line' => __LINE__));
                        $value = utf8_encode($value);
$this->app['monolog']->addInfo(sprintf("key [%s], after encoding [%s]", $key, $value),
array('method' => __METHOD__, 'line' => __LINE__));
                    }
