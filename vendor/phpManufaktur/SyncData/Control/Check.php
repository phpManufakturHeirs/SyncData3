            ...
            if ($table_checksum !== $table['last_checksum']) {
                ...
                if ($table['index_field'] === BackupMaster::NO_INDEX_FIELD) {
                    ...
                    if (false === ($backupStatus = $BackupTables->selectTableByBackupID(self::$backup_id, $table['table_name']))) {
                        $this->app['monolog']->addInfo(sprintf("Found no informations for table %s", $table),
                            array('method' => __METHOD__, 'line' => __LINE__));
                        return false;
                    }
                    $calculate_checksum = '';
...
// this is for extendedWYSIWYG
if(preg_match('~^mod_wysiwyg~i',$table['table_name']) && $General->isModuleInstalled('extendedWYSIWYG'))
{
#    $this->app['monolog']->addInfo(var_export($record['content'],1));
    $record['content'] = html_entity_decode($record['content']);
#    $this->app['monolog']->addInfo(var_export($record['content'],1));
    // recalculate the checksum
    $new_checksum = md5(implode(',',$record));
}
