...
class General {
    ...
    public function update($table, $fields, $data, $replace_cms_url=true)
    {
        ...
        {
            throw new \Exception(sprintf('Invalid data for table [%s] - $data array is empty!',$table));
        }
        try {
    ...
    /**
     * checks if the given module is installed
     *
     * @access public
     * @param  string  $module
     * @return boolean
    ...
    {
        $SQL = sprintf(
            'SELECT * FROM `%saddons` WHERE ( `directory`="%s" OR `name`="%s" )',
            CMS_TABLE_PREFIX, $module, $module
        );
        try {
            $result = $this->app['db']->fetchAll($SQL);
